...
class SettingPage(BasePage):
    btn_setting = get_template("system_function/btn_setting.png", (-0.415, -0.807))
    btn_music = None
    btn_sound = None
    btn_vibrate = None
    ...
    def open_setting(self, retry=2) -> bool:
        """Open settings panel with retry."""
    ...
    def delete_progress(self, word: str = "confirm") -> None:
        ...
        # Ensure we are at home with retry (use force=False to avoid unnecessary actions)
        for attempt in range(3):
            if home.go_home(force=False):
                break
            wrapper.log_warning(f"go_home attempt {attempt+1} failed, retrying...")
            sleep(2)
        else:
            raise AssertionError("delete_progress: failed to navigate to home screen after 3 attempts")
