...
class GamePage(BasePage):
    ...
    def activate_boosters(self) -> None:
        ...
        sleep(2)
        if self.wait_for_element(self.booster[2]):
            self.tap(self.booster[2])
            sleep(2)
            if self.wait_for_element(self.title_booster[2], timeout=3):
                wrapper.log_info("Booster 3 ran out — buying with coin")
                self.tap(self.pay_coin[1])
                sleep(3)
            else:
                wrapper.log_info("Booster 3 available — no need to buy")
