...
def main():
    try:
        ...
        wrapper.log_info("=== TC12: Tap Play inside Daily Mission popup ===")
        setup_unlocked_daily_mission(home_page, cheat, game, ads, setting, target_level=11)
        daily.open_daily_mission_popup()
        if not daily.wait_for_element(daily.btn_play_daily, timeout=5):
            raise AssertionError("Play button not found in popup")
        daily.tap(daily.btn_play_daily)
        sleep(3)
        if not game.wait_for_element(game.booster[0], timeout=5):
            raise AssertionError("Not in game after tapping Play")
        wrapper.log_info("PASS: Entered game from popup")
        wrapper.log_info("=== TC12 PASSED ===")
    except Exception as e:
        wrapper.log_error(f"TC12 failed: {str(e)}\n{traceback.format_exc()}")
