...
def main():
    try:
        ...
        wrapper.log_info("=== TC17: Use 2,5,8 drill boosters ===")
        setup_unlocked_daily_mission(home_page, cheat, game, target_level=11)
        daily.open_daily_mission_popup()
        execute_mission_action(game, cheat, daily, home_page,ads, lucky, "use_booster_drill", random.choice([2,5,10]))
        daily.open_daily_mission_popup()
        if not daily.wait_for_element(daily.btn_collect, timeout=5):
            raise AssertionError("Mission not marked as complete")
        wrapper.log_info("PASS: Used drill boosters")
        wrapper.log_info("=== TC17 PASSED ===")
    except Exception as e:
        wrapper.log_error(f"TC17 failed: {str(e)}\n{traceback.format_exc()}")
