# pixon/pixonwrapper.py - Utility functions for Airtest tests
...
def _get_ocr():
    ...
    if _ocr is None and _ocr_import_error is None:
        try:
            ...
            _ocr_import_error = e
            print("WARNING: PaddleOCR not installed. OCR functions will return empty results.")
...
# =========================
# Swipe functions (Airtest native)
# =========================
@logwrap
def swipe_up():
...
def find_all_text(img_array: np.ndarray) -> list:
    ...
    try:
        if img_array.ndim == 3 and img_array.shape[2] == 4:
            img_bgr = cv2.cvtColor(img_array, cv2.COLOR_BGRA2BGR)  # type: ignore[attr-defined]
        else:
            img_bgr = cv2.cvtColor(img_array, cv2.COLOR_RGB2BGR)   # type: ignore[attr-defined]
        results = ocr_instance.ocr(img_bgr, cls=True)
        if not results or results[0] is None:
            return []
        return [line[1][0] for line in results[0] if line[1][1] >= 0.6]
    except Exception as e:
...
# =========================
# Level badge detection (optional)
# =========================
    ...
    if isinstance(img, Template):
        ...
        if not img.resolution:
            try:
                ...
                h = G.DEVICE.display_info['height']
                img.resolution = (w, h)
            except Exception:          # P2-07 FIX: was bare `except:`
