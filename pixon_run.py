# pixon_run.py - Airtest batch runner
# Usage:
#   single: python pixon_run.py path/to/test.air --device android://... --report reports
#   batch:  python pixon_run.py test1.air test2.air ... --device ... --report reports
...
def run_one(air_name: str, report_root: Path, device_id: str | None) -> bool:
    ...
    if args.recording:
        ...
        if isinstance(args.recording, str) and (
            os.path.isdir(args.recording) or os.path.isfile(args.recording)
        ):
        ...
        scrcpy_exe = current_dir / "pixon" / "scrcpy-win64" / "scrcpy.exe"
...
def generate_summary_report(results: list, report_root: Path):
    html_content = """
    <!DOCTYPE html>
    <html>
    <head>
    ...
    for name, passed in results:
        report_path = report_root / name / "log.html"
        if report_path.exists():
            link = f"{name}/log.html"
        else:
            link = "#"
...
report_root = Path(args.report).resolve() if args.report else current_dir / "reports"
...
for air_name in air_names:
    ...
    passed = run_one(air_name, report_root, device_id)
